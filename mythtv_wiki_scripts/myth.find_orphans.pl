...
#  media files for which there is no matching database entry.
#
...
options:
	...
	--dodbdelete		DELETE DATABASE entries with no matching file (default: don't)
	--dodelete		DELETE FILES with no database entry (default: don't)
...
if ($opt_dodelete) {
    print "WARNING: Running with --dodelete will DELETE RECORDINGS or VIDEO FILES in the supplied directory.\n";
    if (GetAnswer("Are you very sure that you want to DELETE these files?", "n") eq "n") {
        exit 0;
    }
}

if ($opt_dodbdelete) {
    print "WARNING: Running with --dodbdelete will DELETE DATABASE ENTRIES if it cannot find the corresponding file.\n";
    if (GetAnswer("Are you very sure that you want to DELETE the database entries?", "n") eq "n") {
        exit 0;
    }
}
